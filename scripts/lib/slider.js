...
function SliderVerification(selector, options) {
    ...
    options = options || {};
    this.buttonWidth = options.width || '100'; //触发按钮的宽度
    this.buttonBoxWidth = options.width || '92'; //触发按钮的宽度
    this.buttonHeight = options.height || '4.4'; //触发按钮的高度
    // this.baseURL = 'https://open.baiwang.com/defense';
    this.baseURL = '/safety/defense';
...
SliderVerification.prototype = {
    ...
    createDoms: function() {
        ...
        this.sliderButton.css({
            "text-align": 'center',
            "position": 'relative',
            "height": this.buttonHeight + 'rem',
            "width": this.buttonWidth + '%',
            "line-height": this.buttonHeight + 'rem'
        })
        this.box.css({
            "text-align": 'center',
            "position": 'relative',
            "height": this.buttonHeight + 'rem',
            "width": this.buttonBoxWidth + '%',
            "line-height": this.buttonHeight + 'rem'
        })
    },
    domEvent: function() {
        ...
        if(isMobileDevice){
            this.refreshBox.on('touchend', refreshEvent)
        }else {
            this.refreshBox.on('click', refreshEvent)
        }
    ...
    show: function() {
        ...
        var docHide = function(e) {
                ...
                if(isMobileDevice){
                    $(document).on("touchend",docHide);
                }else {
                    $(document).on("click",docHide);
                }
        ...
        // };
        // if(isMobileDevice){
        //     this.box.on("touchend",boxShow)
        // }else{
        //     this.box.on("click",boxShow)
        // }
    ...
    verification: function(startLeft) {
        ...
        $.ajax({
            ...
            .then(function(res) {
                if(res.returnCode == "0") {
                    sessionStorage.setItem('token', res.data.cert);
                    that.success()
                    window.cert = res.data.cert;
                } else {
                    that.fail(startLeft)
                }
            }, function(err) {
    ...
    fail: function(startLeft) {
        ...
        setTimeout(function() {
            that.oButton.removeClass('validate_button_false');
